...
// Define the global system state and dynamic threshold
// (declared as extern in globals.h)
volatile SystemState systemState = STATE_CALIBRATING;
float activeThreshold = 50.0f;  // Safe fallback — overwritten after calibration

void setup() {
    Serial.begin(115200);
    // Serial.println removed — all output goes over WebSocket

    // 1. Create Queues and Semaphores
    rawDataQueue       = xQueueCreate(2048, sizeof(uint16_t));            // Buffered for 1024-pt FFT accumulation
    processedDataQueue = xQueueCreate(8, sizeof(VitalSignData));          // Holds dual-band vital sign data
    ...
    xTaskCreate(vDetectionTask,        "DetectTask",  4096, NULL, 2, NULL);
